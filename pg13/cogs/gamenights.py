...
def gamenight_increments(guild, host_id, db_row):
    point_thresholds = config[str(guild.id)]["thresholds"]
        ...
        async with self.db_pool.acquire() as con:
            if before.channel is not None:
                ...
                left_gamenight = await con.fetchval(
                    ...
                    "UPDATE voice_logs SET duration = duration + (CURRENT_TIMESTAMP - join_time)::INTERVAL WHERE userid = $1 and channel = $2 RETURNING (SELECT * FROM left_gamenight)",
                    before.channel.id,
                    member.id,
                )
